...
extension Rotatable {
  ...
  func rotateAnimationFrom(_ fromItem: UIView, toItem: UIView, duration: Double) {
    ...
    let fromRotate  = animationFrom(0, to: .pi, key: "transform.rotation", duration: duration)
    let fromOpacity = animationFrom(1, to: 0, key: "opacity", duration: duration)

    let toRotate    = animationFrom(-.pi, to: 0, key: "transform.rotation", duration: duration)
